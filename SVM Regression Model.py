...
def generate_data(num_samples, x_range, y_range, z_range):
    ...
    return X, y
# Save the SVM model details to a file
def export_svm_model(svm_model, scaler):
    model_data = {
    ...
    with open('svm_model_data.json', 'w') as f:
...
plt.tight_layout()
plt.show()
export_svm_model(svm_model, scaler)
...
def generate_data(num_samples, x_range, y_range, z_range):
    ...
    y = np.all((X >= [0, 0, 0]) & (X <= [TRUCK_X, TRUCK_Y, TRUCK_Z]), axis=1).astype(int)
    return X, y
# Save the SVM model details to a file (closed for now because of data transfer problems if needed can be included in below)
def export_svm_model(svm_model, scaler):
    model_data = {
        'support_vectors': svm_model.support_vectors_.tolist(),
        'dual_coef': svm_model.dual_coef_.tolist(),
